...
public class CoreBiGramTableDictionary {
    ...
    static {
        ...
        LogUtil.logger.info("开始加载二元词典" + path + ".table");
        long start = System.currentTimeMillis();
        if (!load(path)) {
            throw new IllegalArgumentException("二元词典加载失败");
        } else {
            LogUtil.logger.info(path + ".table" + "加载成功，耗时" + (System.currentTimeMillis() - start) + "ms");
        }
    }
    ...
    static boolean load(String path) {
        ...
        try {
            ...
            LogUtil.logger.info("二元词典读取完毕:" + path + "，构建为TableBin结构");
        } catch (FileNotFoundException e) {
            LogUtil.logger.severe("二元词典" + path + "不存在！" + e);
            return false;
        } catch (IOException e) {
            LogUtil.logger.severe("二元词典" + path + "读取错误！" + e);
            return false;
        }
        LogUtil.logger.info("开始缓存二元词典到" + datPath);
        if (!saveDat(datPath)) {
            LogUtil.logger.warning("缓存二元词典到" + datPath + "失败");
        }
        return true;
    }
...
//            out.close();
            ...
            LogUtil.logger.log(Level.WARNING, "在缓存" + path + "时发生异常", e);
            return false;
        }
...
//
//        int size = byteArray.nextInt(); // 这两个数组从byte转为int花费时间过长
//        start = new int[size];
...
//        }
        ...
        try {
            ...
            LogUtil.logger.warning("尝试载入缓存文件" + path + "发生异常[" + e + "]，下面将载入源文件并自动缓存……");
            return false;
        }
        return true;
    }
